...
export default function ImplementasiPage() {
  return (
    <div className="prose-custom max-w-4xl">
      <p className="!mb-1 text-sm font-semibold uppercase tracking-wider text-indigo-600">BAB IV</p>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {workflowSteps.map(({ icon: Icon, title, description }, idx) => (
          <div key={idx} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <h3 className="!mt-0 !mb-1 text-sm font-semibold text-slate-900">{title}</h3>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border-2 border-indigo-400 bg-indigo-50 p-6 text-center">
          <p className="!mb-1 text-sm font-medium text-indigo-600">ARIMA/SARIMA (Aplikasi)</p>
          <p className="!mb-1 text-4xl font-bold text-indigo-700">4,5%</p>
          <p className="!mb-0 text-sm text-indigo-600">MAPE — Sangat Baik</p>
        </div>
        <div className="rounded-xl border border-slate-200 bg-white p-6 text-center">
          <p className="!mb-1 text-sm font-medium text-slate-500">Excel FORECAST.ETS</p>
          <p className="!mb-1 text-4xl font-bold text-slate-700">12,8%</p>
          <p className="!mb-0 text-sm text-slate-500">MAPE — Baik</p>
        </div>
      </div>
      <div className="mt-4 rounded-xl border border-indigo-200 bg-indigo-50 p-6">
        <p className="!mb-0 font-medium text-indigo-900">
      ...
      <div className="mt-10 flex items-center justify-between">
        <Link href="/modul/metodologi" className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-700 no-underline transition-colors hover:bg-slate-50">
